...
public class Bip39 {
    ...
    */
   private static String[] rawEntropyToWords(byte[] rawEntropy) {
   ...
   //snmp security add for 12 or 24
   public static MasterSeed createRandomMasterSeed(RandomSource randomSource, int length) {
	      byte[] rawEntropy = new byte[length / 8];
	      randomSource.nextBytes(rawEntropy);
	      // We directly use the raw entropy here as key material, as even if urandom has certain kinds
	      // of backdoors, it should be good enough thanks to cryptographic hashing in the seed
	      // generation.
